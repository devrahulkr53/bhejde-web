...
export default function Driver({app}) {
    ...
    const onSubmit = data => {
        ...
        fetch(apiUrl+`drivers.json?auth=${apiAuth}&orderBy="phone"&startAt="${data.phone}"&endAt="${data.phone}"`).then(d=>d.json()).then(json=>{
    ...
    const uploadFiles = async (file) => {
        return new Promise(function(resolve,reject){
            ...
            const storageRef = ref(storage, 'drivers/' + new Date().toISOString() + file.name);
